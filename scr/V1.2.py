...
class TrspGenerator:
    def __init__(self, root):
        self.root = root
        self.root.title("Satellite Transponder Maker v1.2")
        ...
        # Koppla Enter till alla fält i listan
    ...
    def focus_next(self, current_idx):
        if current_idx < len(self.field_order) - 1:
            # Hoppa till nästa textfält
            self.field_order[current_idx + 1].focus_set()
        else:
            # Om vi är på sista fältet, försök lägga till blocket
            if self.btn_add["state"] == "normal":
                self.add_block()
                # Efter tillägg, hoppa tillbaka till "Block Name" för nästa block
    ...
    def validate_all(self):
        ...
        num_keys = ["d_low", "d_high", "u_low", "u_high", "baud"]
        for k in num_keys:
            val = self.inputs[k].get().strip()
            is_bad_num = bool(val and not val.isdigit())
            set_style(self.inputs[k], is_bad_num)
            if is_bad_num: errors = True
        ...
        try:
            ...
            # Uplink check
            u_low = self.inputs["u_low"].get().strip()
            u_high = self.inputs["u_high"].get().strip()
            if u_low and u_high and int(u_high) < int(u_low):
                set_style(self.inputs["u_high"], True)
                errors = True
        except ValueError:
    ...
    def add_block(self):
        ...
        mode = self.inputs["mode"].get().strip().upper() or "FM"
        d_low = self.inputs["d_low"].get().strip()
        d_high = self.inputs["d_high"].get().strip() or d_low
        ...
        lines.append(f"DOWN_LOW={d_low}")
        lines.append(f"DOWN_HIGH={d_high}")
        lines.append(f"UP_LOW={u_low}")
        lines.append(f"UP_HIGH={u_high}")
        ...
        # --- MPORTANT: Add the entire block as ONE string to the list ---
